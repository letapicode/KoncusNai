...
Status: **open** as of September 24, 2026. This record concerns making the existing source repository public. It does not approve a version 1 installer, commercial distribution, or any change to model availability. The owner reports sending inquiries to AI4Bharat, Nyra, and Ampixa; no reply or grant has been recorded. Sent-message copies and recipient delivery have not been independently verified. The owner has directed preparation of public work-in-progress source visibility without waiting for those replies or qualified license review. That is a publication-risk decision in progress, not a finding that third-party rights are cleared or final approval to change visibility. The [source/runtime rights review packet](public-source-rights-review-packet.md) describes the exact current distribution and questions for qualified review.
...
**Owner decision needed:** The owner wants to prepare public source visibility with the existing optional Indic Parler integration before an upstream reply or qualified advice. A final visibility decision must acknowledge that the named-voice, dataset, and output questions are unresolved. A notice telling users to review terms does not itself resolve the publisher's rights questions.
...
The unanswered upstream emails do not prevent technical preparation. The reviewed source-only code and static packaging baseline is technically ready, but no email, user notice, or green build grants additional rights. The owner has chosen to prepare integration-source visibility while the provider replies and qualified GPL/LGPL/FFmpeg review remain pending and every model option remains available. This direction does not mark the rights questions resolved. Before changing `letapicode/KoncusNai` from private to public, review the final committed file list and fresh history, obtain a passing private CI run for the final documentation commit, inspect the private Actions logs and retained artifacts that would become visible, and obtain the owner's explicit final approval informed by the remaining uncertainty. Label any public repository **source-available, noncommercial, work in progress**. Immediately after visibility changes, enable GitHub private vulnerability reporting and update `SECURITY.md` with the real advisory route. Ordinary model use, generated output, commercial use, and a bundled/supported installer are distinct decisions. No tag, installer, release artifact, or version 1 claim is part of this decision.
